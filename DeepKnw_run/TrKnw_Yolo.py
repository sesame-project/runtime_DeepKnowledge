...
class KnowledgeCoverage:
    ...
    def KnwTransferYolo(self, test_inputs, statep):
        ...
        # for input_index in (range(self.BATCH)):
                # T = next(iter(test_inputs))
                # x = T['img']
        for input_index, dt in enumerate(test_inputs):
    ...
    def idc_pipline(self, model_name, train_inputs, subject_layer, test_inputs, selected_class, Tr_knw_neuron,
        ...
        coverage, covered_combinations, max_comb, df_details = measure_idcYolo(self.model,model_ul, model_name,
                                                               ...
                                                               Tr_knw_neuron, subsetTop,self.data_set,self.BATCH, self.samp,self.DATA_DIRe,self.classes,y_test,
                                                               covered_combinations)
    ...
    def runYolo(self, split, typTrknw, data_path, val_path, OOD_data, OOD_yaml_path):
        corrupted=False
        ...
        print("total samples for testset",len(yolo_testset))
        print("test loader size",len(test_loader))
        print("number iterations", math.ceil(len(yolo_testset)/self.BATCH))
        iterations=math.ceil(len(yolo_testset)/self.BATCH)
        # for i, dt in enumerate(test_loader):
        #     if (i+1)%5==0:
        #         print(f'steps {i+1}/ {iterations}, data {len(dt)}')
                # print(dt['im_file'])


        if corrupted==True:
            corrupt_loader = FinalDataLoaderCorrupted(yolo_testset, self.testpath, batch=self.BATCH, corr_type=['gaussian_blur', 'brightness'])
            n_batches = len(corrupt_loader)
            print(" size of corruptdata ",n_batches)

            test_samples=corrupt_loader
        else:
            test_samples = test_loader
        # nc_test = self.KnwTransferYolo(test_samples,  "testing")
        #
        ...
        Knw_coverage, number_covered, covered_combinations, max_comb = self.coverage_score(
            TopN, test_data, val_loader, test_samples,test_samples.dataset.labels, subsetTop)
        ...
        return Knw_coverage, number_covered, covered_combinations, max_comb, len(test_loader), len(test_loader), TopN