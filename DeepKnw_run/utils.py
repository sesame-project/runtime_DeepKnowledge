...
def get_relevantneurons(relevant_neurons):
    ...
    for x in relevant_neurons:
        if x[0] in relevant:
            ...
            relevant[x[0]] = [x[1]]
    return relevant
def pre_processTCC(testfile, corruptedfile):

    TT = pd.read_pickle(corruptedfile)
    TCC = TT.loc[TT['prediction'] == False]
    F_TCC = TCC['combinations'].value_counts()
    F_TCC = pd.DataFrame(F_TCC)
    F_TCC = F_TCC.rename(columns={"count": "Uncorrect"})
    F_TCC = pd.DataFrame(F_TCC)
    TrueT = TT.loc[TT['prediction'] == True]
    T_TCC = TrueT['combinations'].value_counts()
    T_TCC = pd.DataFrame(T_TCC)
    T_TCC = T_TCC.rename(columns={"count": "Correct"})
    T_TCC = pd.DataFrame(T_TCC)
    CorruptedTCC = pd.concat([F_TCC, T_TCC], axis=0)
    CC = pd.read_pickle(testfile)
    C = CC.loc[CC['prediction'] == False]
    F_CC = C['combinations'].value_counts()
    F_CC = pd.DataFrame(F_CC)
    F_CC = F_CC.rename(columns={"count": "Uncorrect"})
    F_CC = pd.DataFrame(F_CC)
    TrueC = CC.loc[CC['prediction'] == True]
    T_CC = TrueC['combinations'].value_counts()
    T_CC = pd.DataFrame(T_CC)
    T_CC = T_CC.rename(columns={"count": "Correct"})
    T_CC = pd.DataFrame(T_CC)

    testTCC = pd.concat([F_CC, T_CC], axis=1)
    all = pd.concat([CorruptedTCC, testTCC], ignore_index=False, sort=False)
    all = all.fillna(0)
    all = all.reset_index()
    keys = all.combinations
    Uncorrect = all.Uncorrect
    Correct = all.Correct
    comb = []
    for i in keys:
        comb.append(tuple(i))
    values = list(zip(Correct, Uncorrect))
    TCC= dict(zip(comb, values))
...
def get_designTime(dataset, DATA_DIR,model_name):
    ...
    Testset_details_dir = os.path.join(DATA_DIR, 'TCC_details' + "_" +'Testset'+ "_" +dataset + "_" + model_name + '_.sav')
    Corrupted_details_dir = os.path.join(DATA_DIR, 'TCC_details' + "_" +'Corrupted'+ "_" + dataset + "_" + model_name + '_.sav')
    ...
    Testset_TCC=pre_processTCC(Testset_details_dir, Corrupted_details_dir)
...
def get_allouts(model, X_set):
    ALL_outs=[]
    # for input_index in (range(BATCH)):
    # T = next(iter(X_set))
    for input_index, dt in enumerate(X_set):
        T = dt
        # x = T['img']
