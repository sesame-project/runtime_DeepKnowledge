...
class DeepKnw:
    ...
    def estimate_coverage(self, testloader):
        startTime = time.time()
        ...
        if approach == 'knw':
            ...
            knw = KnowledgeCoverage(model, dataset, model_name, subject_layer, trainable_layers, dense_layers, method,
                                    percent, threshold, attack, skip_layers, nbr_Trknw, self.config_module,
                                    self.classes, selected_class=1)

            Knw_coverage, covered_TrKnw, combinations, max_comb, testsize, zero_size, Trkneurons = \
                knw.runYolo(split, TypeTrknw, self.yaml_path, self.val_path, self.OOD_data, self.OOD_yaml_path)

            print(
                "The test set coverage: %.2f%% for dataset  %s and the model %s " % (Knw_coverage, dataset, model_name))
    ...
    def DesignDataAnalyzer(self):
        ...
        new_data = {'train_outs': train_outs, 'relevant_neurons': relevant_neurons, 'Testset_TCC': Testset_TCC,
                    'Train_qtizer': Train_qtizer}
    ...
    def Runtime_Estimate(self, img_frame, model):
        ...
        try:
            ...
            print("Error: Cannot Open Data Folder.\n")
            sys.exit(1)
        ...
        KNW_runtime = measure_Runtime(model, model_name, resized_tensor, self.dataAnalyser['train_outs'],
                                      self.dataAnalyser['relevant_neurons'], self.dataAnalyser['Testset_TCC'],
                                      self.dataAnalyser['Train_qtizer'], self.dataset, self.DATA_DIR,
                                      covered_combinations)
